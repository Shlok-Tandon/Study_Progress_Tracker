        ...
        builder: (context, snap) {
          ...
          final podiumEntries = [
            for (var i = 0; i < podiumCount; i++)
              PodiumEntry(name: (docs[i].data() as Map<String, dynamic>)['name'] ?? 'User', streak: (docs[i].data() as Map<String, dynamic>)['streak'] ?? 0),
          ];
          ...
          return ListView(
            ...
            children: [
              ...
              for (var i = podiumCount; i < docs.length; i++)
                _LeaderboardRow(rank: i + 1, data: docs[i].data() as Map<String, dynamic>, game: game, isTopThree: !hasPodium && i < 3),
            ],
          );
        },
      ),
...
class _LeaderboardRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final badges = data['badgeCount'] ?? 0;
    ...
    if (isTopThree) {
      card = card.animate(onPlay: (c) => c.repeat()).shimmer(duration: 1800.ms, delay: 700.ms, color: scheme.primary.withOpacity(0.22));
    }
    return card;
  }
}